...
@IBDesignable class QuestionViewiPhone5: UIView {
    ...
    @IBAction func SjekkButton(_ sender: UIButton) {
        if (SjekkButton.titleLabel?.text == "Sjekk"){
            ...
            SjekkButton.setTitle("Neste", for: UIControlState.normal)
            ...
            SjekkButton.setTitle("Avslutt", for: UIControlState.normal)
            if (rightAnswers >= Int(Float(spm.numberQuestions)*0.8)){
                RightOrWrong.text = "Gratulerer du bestod!"
                QuestionText.text = "Du fikk \(rightAnswers) av \(spm.numberQuestions) spørsmål riktige."
            }
            else {
            ...
            addQuestion()
            RightOrWrong?.text = ""
            SjekkButton.setTitle("Sjekk", for: UIControlState.normal)
    ...
    func setUpQuestions(numQuestions: Int, isArtQuestion: Bool){
    ...
    func addQuestion(){
        ...
        optArray[0]?.setTitle(spm?.getWrongAnswer1(), for: UIControlState.normal)
        optArray[1]?.setTitle(spm?.getWrongAnswer2(), for: UIControlState.normal)
        optArray[2]?.setTitle(spm?.getAnswer(), for: UIControlState.normal)
    ...
    func loadViewFromNib() -> UIView {
       // let bundle = NSBundle(forClass:self.dynamicType)
        ...
        let nib = UINib(nibName: "QuestionViewiPhone5", bundle: bundle)
